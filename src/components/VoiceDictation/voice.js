...
export default class IatRecorder {
    constructor(opts = {
        ...
        this.onWillStatusChange = opts.onWillStatusChange;
    ...
    getWebSocketUrl() {
        return new Promise((resolve, reject) => {
            ...
            var signatureSha = HmacSHA256(signatureOrigin, this.apiSecret);
        ...
        try {
            if (!self.appId || !self.apiKey || !self.apiSecret) {
                ...
                if (typeof (Worker) !== "undefined") {
                    // self.webWorker = new Worker('./worker.js');
                    // self.webWorker = new Worker('https://ihubbest.smgtech.net/static/scripts/worker.js');
                    self.webWorker = new Worker(require('../../assets/worker.js'));
                    // self.webWorker = new Worker(import('./worker.js'));

                    // var blob = new Blob(['./worker.js']);
                    // self.webWorker = new Worker(window.URL.createObjectURL(blob));
        ...
        let getMediaFail = (e) => {
            switch (e.message || e.name) {
                ...
                case 'Requested device not found':
                    console.error('未识别到麦克风设备！');
                    break;
                default:
                    console.error('无法打开麦克风，异常信息:' + (e.code || e.name));
                    break;
            };
